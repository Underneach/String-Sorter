...
def Sorting(self, file_path: str) -> None:
    ...
    with (open(file_path, 'r', encoding=self.encoding, buffering=4096) as file,
          Pool(processes=self.processor_count) as pool,
          tqdm.tqdm(total=string_count, unit=' Strings', desc=f'[*] Обработка файла', colour='blue',
                    dynamic_ncols=True, smoothing=True,
                    bar_format='{desc} | {bar} |{percentage:3.0f}% | {n_fmt}/{total_fmt} | {rate_fmt}') as progress_bar):
